...
def clean_excel_id(value):
    """Pomoćna funkcija koja čisti ID-eve od .0 i nan vrijednosti."""
    if pd.isna(value) or str(value).lower() in ['nan', 'none', '', ' ']:
        return ""
    
    str_val = str(value).strip()
    
    # Ako završava s .0 (npr "100.0"), makni to
...
def render_hr_view():
    ...
    # --- 4. ŠIFARNIK & KORISNICI ---
        ...
        with t2:
            ...
            with st.form("hr_add"):
                ...
                if st.form_submit_button("Dodaj"): 
                    try: 
                        clean_id = clean_excel_id(i) # KORISTIMO NOVU FUNKCIJU
        ...
        # TAB 3: IMPORT (POPRAVLJENA LOGIKA ZA 100.0 I NAN)
        with t3:
            ...
            if f and st.button("Importiraj"):
                try:
                    ...
                    for _, r in df.iterrows():
                        # Čistimo ID zaposlenika (Column 0)
                        kid = clean_excel_id(r[0])
                        
                        if not kid or kid.lower() in ['id', 'kadrovski', 'kadrovski broj']: continue
                        
                        # Čistimo Manager ID (Column 4)
                        val4 = ""
                        if len(r) > 4:
                            val4 = clean_excel_id(r[4])
