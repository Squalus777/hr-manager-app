...
def render_hr_view():
    ...
    # --- 4. ŠIFARNIK & KORISNICI ---
        ...
        # TAB 3: IMPORT (AŽURIRANO ZA ROLU!)
        with t3:
            ...
            if f and st.button("Importiraj"):
                try:
                    ...
                    for _, r in df.iterrows():
                        ...
                        kid = str(r[0]).strip()
                        val4 = str(r[4]) if len(r)>4 else None # Manager
                        val5 = str(r[5]).strip() if len(r)>5 else "Employee" # ROLA (Kolona F)
                        ...
                        if auto_create:
                            # Provjera postoji li user, ako ne kreiraj s rolom iz Excela (ili default Employee)
    ...
    # --- 5. POSTAVKE RAZDOBLJA ---
        ...
        with t1:
            with st.form("np"):
                ...
                if st.form_submit_button("Kreiraj"): 
                    try: conn.cursor().execute("INSERT INTO periods VALUES (?,?)", (n, str(d))); conn.commit(); st.success("Kreirano")
        ...
        with t2:
            ...
            if ps:
                ...
                if st.button("Postavi AKTIVNO"): conn.cursor().execute("UPDATE app_settings SET setting_value=? WHERE setting_key='active_period'", (s,)); conn.commit(); st.rerun()
        with t3:
            if current_period:
                curr = conn.cursor().execute("SELECT deadline FROM periods WHERE period_name=?", (current_period,)).fetchone()
                nd = st.date_input("Novi rok", value=datetime.strptime(curr[0], "%Y-%m-%d").date() if curr else date.today())
                if st.button("Ažuriraj"): conn.cursor().execute("UPDATE periods SET deadline=? WHERE period_name=?", (str(nd), current_period)); conn.commit(); st.success("OK")
