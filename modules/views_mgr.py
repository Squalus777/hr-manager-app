...
# Importamo make_hashes za kreiranje lozinki
...
def render_manager_view():
    ...
    if menu == "📊 Dashboard":
        ...
        with t1:
            ...
            else: st.info("Još nema unesenih procjena za ovaj period.")
        with t2:
    ...
    # ------------------------------------------------------------------
        ...
        for _, r in emps.iterrows():
            ...
            msg = f"⚠️ PAŽNJA: Ukupna težina je {total_w}% (Mora biti 100%)" if total_w != 100 else "✅ Ciljevi ispravno postavljeni (100%)"
            msg_color = "red" if total_w != 100 else "green"
                        ...
                        with st.form(f"fg_{gid}"):
                            ...
                            if st.form_submit_button("Obriši"):
                                with sqlite3.connect(DB_FILE) as d_conn:
                                    d_conn.execute("DELETE FROM goal_kpis WHERE goal_id=?", (gid,)); d_conn.execute("DELETE FROM goals WHERE id=?", (gid,)); d_conn.commit()
                                st.rerun()
                ...
                if st.form_submit_button("Dodaj u sustav"):
                    try:
                        with sqlite3.connect(DB_FILE) as i_conn:
                            ...
                            # 1. Insert u Master
                            i_conn.execute("INSERT INTO employees_master VALUES (?,?,?,?,?)", (clean_id, nn, nr, nd, st.session_state['username']))
                            
                            # 2. Insert u Users (Automatski login)
                            default_pass = make_hashes("user123")
                            i_conn.execute("INSERT OR IGNORE INTO users (username, password, role, department) VALUES (?,?,?,?)", 
                                          (clean_id, default_pass, 'Employee', nd))
                            
                            i_conn.commit()
                        st.success(f"Dodan {nn} i kreiran login: {clean_id} / user123!"); time.sleep(2); st.rerun()
        ...
        for _, emp in my_team.iterrows():
            ...
            if not exist.empty:
                ...
                if status_val == 'Submitted':
                    is_locked = True; status_icon = "🔒"; status_text = f"Završeno ({r['category']})"
                else: status_icon = "✏️"; status_text = f"U tijeku ({r['category']})"
            else: status_icon = "⚠️"; status_text = "Nije započeto"
            ...
            with st.expander(f"{status_icon} {emp['ime_prezime']} ({status_text})"):
                if is_locked and r is not None:
                    ...
                    with st.form(f"edit_eval_{kid}"):
                        ...
                        if st.form_submit_button("💾 Spremi kao nacrt"):
                            with sqlite3.connect(DB_FILE) as s_conn:
                                ...
                                s_conn.execute("UPDATE evaluations SET p1=?,p2=?,p3=?,p4=?,p5=?, pot1=?,pot2=?,pot3=?,pot4=?,pot5=?, avg_performance=?, avg_potential=?, category=?, action_plan=?, status='Draft' WHERE id=?", (*nps, *npots, ap, at, calculate_category(ap, at), npl, eval_id_int))
                                s_conn.commit()
                    ...
                    with col_pdf:
                         if f"prt_eval_{r['id']}" not in st.session_state: st.session_state[f"prt_eval_{r['id']}"] = False
                         if st.button("🖨️ PDF (Pregled)", key=f"btn_prt_{r['id']}"): st.session_state[f"prt_eval_{r['id']}"] = True; st.rerun()
                    with col_submit:
                        ...
                        if st.button("🔒 ZAKLJUČAJ I POŠALJI", key=f"btn_send_{r['id']}", disabled=not locked_check, type="primary"):
                             try:
                                 with sqlite3.connect(DB_FILE) as lock_conn:
                                     ...
                                     lock_conn.commit()
                                     st.toast("✅ Zaključano!", icon="🔒"); time.sleep(1); st.rerun()
                    ...
                    with st.form(f"new_eval_{kid}"):
                        ...
                        if st.form_submit_button("Spremi Nacrt"):
                            with sqlite3.connect(DB_FILE) as n_conn:
                                ...
                                n_conn.execute("INSERT INTO evaluations (period, kadrovski_broj, ime_prezime, radno_mjesto, department, manager_id, p1,p2,p3,p4,p5, pot1,pot2,pot3,pot4,pot5, avg_performance, avg_potential, category, action_plan, status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (current_period, kid, emp['ime_prezime'], emp['radno_mjesto'], emp['department'], st.session_state['username'], *ps, *pots, ap, at, calculate_category(ap, at), pl, 'Draft'))
                                n_conn.commit()
                            st.success("Kreirano"); st.rerun()
                
                if r is not None:
                    if f"prt_eval_{r['id']}" in st.session_state and st.session_state[f"prt_eval_{r['id']}"]:
                            html_content = f"""<html><head><style>@media print {{ body {{ font-family: Arial, sans-serif; }} .no-print {{ display: none; }} }} body {{ font-family: Arial, sans-serif; padding: 20px; }} .header {{ border-bottom: 2px solid #E2001A; padding-bottom: 10px; margin-bottom: 20px; }} .title {{ color: #E2001A; font-size: 24px; font-weight: bold; }} .meta {{ font-size: 12px; color: #666; }} table {{ width: 100%; border-collapse: collapse; margin-bottom: 20px; }} td, th {{ border: 1px solid #ddd; padding: 8px; text-align: left; }} .box-title {{ background-color: #f0f0f0; padding: 10px; font-weight: bold; margin-top: 15px; border-left: 5px solid #E2001A; }} .print-btn {{ background-color: #E2001A; color: white; border: none; padding: 10px 20px; font-weight: bold; cursor: pointer; border-radius: 4px; margin-top: 20px; }}</style><script>window.onload = function() {{ window.print(); }}</script></head><body><div class="no-print" style="text-align:center;"><button class="print-btn" onclick="window.print()">🖨️ PRINT / SPREMI PDF</button></div><div class="header"><div class="title">OBRAZAC PROCJENE UČINKA</div><div class="meta">Period: {current_period} | Datum: {date.today()}</div></div><table><tr><td><b>Zaposlenik:</b> {r['ime_prezime']}</td><td><b>Pozicija:</b> {r['radno_mjesto']}</td></tr><tr><td><b>Odjel:</b> {r['department']}</td><td><b>Voditelj:</b> {r['manager_id']}</td></tr></table><div class="box-title">1. UČINAK</div><table><tr><th>Kompetencija</th><th>Ocjena</th></tr><tr><td>1. KPI</td><td>{r['p1']}</td></tr><tr><td>2. Kvaliteta</td><td>{r['p2']}</td></tr><tr><td>3. Stručnost</td><td>{r['p3']}</td></tr><tr><td>4. Odgovornost</td><td>{r['p4']}</td></tr><tr><td>5. Suradnja</td><td>{r['p5']}</td></tr><tr style="background:#f9f9f9"><td><b>PROSJEK:</b></td><td><b>{r['avg_performance']}</b></td></tr></table><div class="box-title">2. POTENCIJAL</div><table><tr><th>Kompetencija</th><th>Ocjena</th></tr><tr><td>1. Agilnost</td><td>{r['pot1']}</td></tr><tr><td>2. Autoritet</td><td>{r['pot2']}</td></tr><tr><td>3. Šira slika</td><td>{r['pot3']}</td></tr><tr><td>4. Ambicija</td><td>{r['pot4']}</td></tr><tr><td>5. Stabilnost</td><td>{r['pot5']}</td></tr><tr style="background:#f9f9f9"><td><b>PROSJEK:</b></td><td><b>{r['avg_potential']}</b></td></tr></table><div class="box-title">3. 9-BOX STATUS</div><div style="padding:15px; border:1px solid #ddd; text-align:center; font-size:18px; font-weight:bold;">{r['category']}</div><div class="box-title">4. KOMENTAR</div><div style="padding:15px; border:1px solid #ddd;">{r['action_plan'] if r['action_plan'] else "Nema komentara."}</div><br><br><table style="border:none"><tr><td style="border:none; border-top:1px solid #000;">Potpis zaposlenika</td><td style="border:none;"></td><td style="border:none; border-top:1px solid #000; text-align:right;">Potpis voditelja</td></tr></table></body></html>"""
                            components.html(html_content, height=800, scrolling=True)
                            if st.button("Zatvori pregled", key=f"close_{r['id']}"): st.session_state[f"prt_eval_{r['id']}"] = False; st.rerun()
    ...
    # ------------------------------------------------------------------
        ...
        if sel:
            ...
            with st.expander(f"📄 {emp_name} ({cat})", expanded=True):
                ...
                if len(hist_idp) > 1:
                    with st.popover("📜 Pregledaj stare planove"): st.dataframe(hist_idp, hide_index=True)
